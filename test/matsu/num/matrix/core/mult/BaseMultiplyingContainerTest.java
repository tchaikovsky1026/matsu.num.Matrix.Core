...
/**
 * {@link BaseMultiplyingContainer} のテスト.
 */
@RunWith(Enclosed.class)
final class BaseMultiplyingContainerTest {
     ...
     * 右からスタート, 左からスタート, 右のみ, 左のみ, 無し の5パターンで試す.
     * ========================
     */
    ...
    public static class 行列積のテスト {
        ...
        @Theory
        public void test_演算無しパターン(MatrixDimension matrixDimension) {
            EntryReadableMatrix baseSrc = createRandomMatrix(matrixDimension);
            EntryReadableMatrix base = BaseMultiplyingContainer.basedOn(baseSrc).build();

            testEntryEquality(base, baseSrc, 1E-14);
        }

        @Theory
        public void test_左演算のみパターン(MatrixDimension matrixDimension) {
            EntryReadableMatrix baseSrc = createRandomMatrix(matrixDimension);
            ...
            var con = BaseMultiplyingContainer.basedOn(baseSrc);
            con.operateLeftSide(l1);
            con.operateLeftSide(l2);
            EntryReadableMatrix result = con.build();

            Matrix expected = Matrix.multiply(l2, l1, baseSrc);

            testEntryEquality(result, expected, 1E-14);
        }

        @Theory
        public void test_右演算のみパターン(MatrixDimension matrixDimension) {
            EntryReadableMatrix baseSrc = createRandomMatrix(matrixDimension);
            ...
            Matrix expected = Matrix.multiply(baseSrc, r1, r2);

            testEntryEquality(result, expected, 1E-14);
        }

        @Theory
        public void test_左から右のパターン(MatrixDimension matrixDimension) {
            EntryReadableMatrix baseSrc = createRandomMatrix(matrixDimension);
            Matrix l = createRandomMatrix(matrixDimension.leftSquareDimension());
            Matrix r = createRandomMatrix(matrixDimension.rightSquareDimension());

            var con = BaseMultiplyingContainer.basedOn(baseSrc);
            con.operateLeftSide(l);
            con.operateRightSide(r);
            EntryReadableMatrix result = con.build();

            Matrix expected = Matrix.multiply(l, baseSrc, r);

            testEntryEquality(result, expected, 1E-14);
        }

        @Theory
        public void test_右から左のパターン(MatrixDimension matrixDimension) {
            EntryReadableMatrix baseSrc = createRandomMatrix(matrixDimension);
            Matrix l = createRandomMatrix(matrixDimension.leftSquareDimension());
            Matrix r = createRandomMatrix(matrixDimension.rightSquareDimension());
